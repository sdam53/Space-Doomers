class SceneManager {
  ...
  loadLevel(title, transition) {
    ...
    if (!title && !transition && !this.gameOver) {
      ...
      if (typeof MAPONE.DOOR[0] != 'undefined') {
        for (let i = 0; i < MAPONE.DOOR[0].length; i ++) {
          this.game.addDoor(new Door(this.game, MAPONE.DOOR[0][i] * 125, MAPONE.DOOR[1][i] * 125));
        }
      }
    }
  ...
  loadLevelTwo() {
    ...
    if (!this.gameOver) {
      for (let i = 0; i < MAPTWO.MAP.length; i++) { //create level
        for (let j = 0; j < MAPTWO.MAP[0].length; j++) {
          if (MAPTWO.MAP[i][j] === 1) {
            this.game.addTile(new Ground(this.game, x, y, 125, 125, 1));
          } else if (MAPTWO.MAP[i][j] === 0) {
            this.game.addTile(new Wall(this.game, x, y, 125, 125, 1));
          } else if (MAPTWO.MAP[i][j] === 2) {
            this.game.addTile(new Trap(this.game, x, y, 125, 125, 1));
          }
          x += 125;
        }
    ...
    //adding player
    this.player = new Player(this.game, MAPTWO.PLAYER[0] * 125, MAPTWO.PLAYER[1] * 125);
    this.game.entities.player = this.player;
  }
